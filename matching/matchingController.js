...
// let stud1 = new Student("segev5115", "passw0rd", "Student", [5, 6], [1, 2]);
// let stud2 = new Student("lucy", "p", 1, 1, [2, 1]);
...
const getMatching = asyncHandler(async (req, res) => {
    ...
    for (let p = 0; p < res2.length; p++) {
        let res3 = await db.manyOrNone("SELECT studentUsername, professorPreferences as preferences FROM sectionsApplicants WHERE id = $1", res2[p].id)
        .catch((err) => {
            console.log(err);
        });

        const comparePref = function(a, b){
    ...
    if (typeof(match) === 'string'){
        ...
        for (let i = 0; i < match.length; i++) {
            for (let j = 0; j < match[i].length; j++) {
                await db.none("INSERT INTO matchings(section, username) VALUES ($1, $2)", [res2[i].id, match[i][j]])
                .catch((err) => {
                    console.log(err);
                });

                await db.none("UPDATE applicationInfo SET status = $1 WHERE username = $2", [0, match[i][j]])
                .catch((err) => {
                    console.log(err);
                });
